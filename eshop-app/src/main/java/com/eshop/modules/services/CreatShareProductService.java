...
public class CreatShareProductService {
     ...
     */
    public StoreOrderQueryVo handleQrcode(StoreOrderQueryVo storeOrder, String path){
        //门店
        if(OrderInfoEnum.SHIPPIING_TYPE_2.getValue().equals(storeOrder.getShippingType())){
            ...
            if(StrUtil.isBlank(mapKey)) {
                throw new EshopException("请配置腾讯地图key");
            }
     ...
     * @return url
     */
    public String getSpreadUrl(String from, ShopUser userInfo, String siteUrl,
     ...
     * @return url
     */
    public String getBargainPosterUrl(Long bargainId, ShopUser userInfo, String siteUrl,
                                      String apiUrl, String path, String from){
        Long uid = userInfo.getUid();
        StoreBargain storeBargainQueryVo = storeBargainService
                .getById(bargainId);
     ...
     * @return url
     */
    public String getPinkPosterUrl(Long pinkId, ShopUser userInfo, String siteUrl,
                                   String apiUrl, String path, String from){
        Long uid = userInfo.getUid();
        StorePink storePink = storePinkService.getById(pinkId);
        if(ObjectUtil.isNull(storePink)) {
            throw new EshopException("拼团不存在");
        }
        StoreCombination storeCombination = storeCombinationService.getById(storePink.getCid());
        if(ObjectUtil.isNull(storeCombination)) {
            throw new EshopException("拼团产品不存在");
        }
...
//                        FileUtil.file(picImage));
     ...
     */
    public  String creatProductPic(StoreProduct productDTO, String shareCode, String spreadPicName, String spreadPicPath, String apiUrl) throws IOException, FontFormatException {
        SystemAttachment attachmentT = systemAttachmentService.getInfo(spreadPicName);
        String spreadUrl = "";
        if(ObjectUtil.isNull(attachmentT)){
            //创建图片
            ...
            try {
                priductUrl = ImageIO.read(new URL(com.eshop.utils.FileUtil.transformStyle(productDTO.getImage())));
            } catch (IOException e) {
                e.printStackTrace();
            }
